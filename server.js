...
app.get("/hyperlanes/search/:search_query?", (req, res, next) => {
	var search_query = (req.params == null || req.params.search_query == null) ? "" : decodeURIComponent(req.params.search_query);
	var sql_search_query = "%" + search_query + "%";
	mysql.pool.query("SELECT * FROM (" + HYPERLANES_QUERY + ") AS h WHERE h.system1Name LIKE ? OR h.system2Name LIKE ?", [sql_search_query, sql_search_query], (err, rows, fields) => {
		if(err) {
			res.status(500).send(err);
		} else {
			var result = {
